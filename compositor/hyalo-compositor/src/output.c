...
handle_output_destroy(struct wl_listener *listener, void *data)
{
	...
	hyalo_wallpaper_output_destroy(output);
...
handle_new_output(struct wl_listener *listener, void *data)
{
	...
	hyalo_wallpaper_output_init(output);

	/*
	 * Set the z-positions to achieve the following order (from top to
	 * bottom):
	 *	- Hyalo wallpaper tree
	 *	- session lock layer
	 *	- window switcher osd
	 *	- (compositor menu)
	 *	- layer-shell popups
	...
	if (output->hyalo_wallpaper_tree) {
		wlr_scene_node_place_below(&output->hyalo_wallpaper_tree->node,
